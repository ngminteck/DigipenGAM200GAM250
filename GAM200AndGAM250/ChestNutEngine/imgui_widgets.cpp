...
bool ImGui::ThumbNail(ImTextureID user_texture_id, const ImVec2& size, std::string* label, bool double_click, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col )
{
	...
	std::string label_to_display = label->substr(0, label->find("##"));

	
	RenderTextClipped(offset.Min + style.FramePadding, offset.Max - style.FramePadding, label_to_display.c_str(), NULL, &label_size, style.ButtonTextAlign, &offset);
...
bool ImGui::DragScalarInput(const char* label, ImGuiDataType data_type, void* v, float v_speed, const void* v_min, const void* v_max, const void* step , const void* step_fast , const char* format , float power, ImGuiInputTextFlags extra_flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
    ...
    bool value_changed = DragBehavior(id, data_type, v, v_speed, v_min, v_max, format, power);
	if (value_changed)
		MarkItemEdited(id);
	...
	if (step != NULL)
	{
		...
		if (InputText("", buf, IM_ARRAYSIZE(buf), extra_flags)) // PushId(label) + "" gives us the expected ID from outside point of view
			value_changed = DataTypeApplyOpFromText(buf, g.InputTextState.InitialText.Data, data_type, v, format);
		PopItemWidth();
		...
		{
			DataTypeApplyOp(data_type, '-', v, v, g.IO.KeyCtrl && step_fast ? step_fast : step);
			value_changed = true;
		}
		SameLine(0, style.ItemInnerSpacing.x);
		if (ButtonEx("+", ImVec2(button_size, button_size), ImGuiButtonFlags_Repeat | ImGuiButtonFlags_DontClosePopups))
...
bool ImGui::DragScalarInputN(const char* label, ImGuiDataType data_type, void* v, int components, float v_speed, const void* v_min , const void* v_max , const void* step , const void* step_fast , const char* format , float power , ImGuiInputTextFlags extra_flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	{
		PushID(i);
		value_changed |= DragScalarInput("##v", data_type, v, v_speed, v_min, v_max, step, step_fast, format, power, extra_flags);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
		v = (void*)((char*)v + type_size);
