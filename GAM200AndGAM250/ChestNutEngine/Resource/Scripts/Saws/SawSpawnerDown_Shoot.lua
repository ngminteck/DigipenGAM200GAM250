...
function Update()
	...
	sprite:SetSprite("SAW_SPAWNER_FIRE_FACEDOWN", states)
  ...
  if Saw1:GetActive() == true then 
    Saw1_aabb:SetOffset(vec2.new(Saw1_aabb:GetOffset().x,_G.mod7*-speed+Saw1_aabb:GetOffset().y))
  end
  
  if Saw2:GetActive() == true then 
    Saw2_aabb:SetOffset(vec2.new(Saw2_aabb:GetOffset().x,_G.mod8*-speed+Saw2_aabb:GetOffset().y))
  end
end